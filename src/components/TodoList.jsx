  ...
  return (
    <div className="bg-gray-800 rounded-2xl shadow-lg hover:shadow-blue-900/20 hover:shadow-xl p-6 w-full max-w-md border border-gray-700 hover:border-blue-900 transition-all duration-300">
      <div className="flex items-center justify-between mb-4">
        ...
        {todos.length > 0 && (
          <button onClick={() => setTodos([])} className="text-xs text-red-400 hover:text-red-300 transition-colors">
            Clear all
          </button>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2 mb-6">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={handleAddTodo} className="bg-blue-600 hover:bg-blue-500 text-white font-medium px-4 py-2 rounded-lg transition-colors">
            Add
          </button>
        </div>
      </div>
          ...
          {todos.map((todo) => (
            <li key={todo.id} className="flex items-center justify-between bg-gray-700 hover:bg-gray-600 rounded-lg px-4 py-3 group transition-colors duration-200">
              <span
                ...
                className={`flex-1 cursor-pointer text-sm transition-all ${todo.completed ? 'line-through text-gray-500' : 'text-white'}`}
              >
                {todo.text}
              </span>
              <button onClick={() => handleRemove(todo.id)} className="ml-3 text-red-400 hover:text-red-300 text-xs opacity-0 group-hover:opacity-100 transition-opacity">
                ✕
              </button>
            </li>
          ))}
      ...
      {todos.length > 0 && (
        <div className="mt-4">
          ...
          <p className="text-gray-500 text-xs mt-2 text-right">{completed} / {todos.length} completed</p>
        </div>
      )}
    </div>
  );
